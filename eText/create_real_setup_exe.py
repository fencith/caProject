#!/usr/bin/env python3
"""
创建真正的 Windows setup.exe 安装包
Create Real Windows setup.exe Installer
使用 Python 脚本创建真正的可执行安装程序
"""
...
class RealSetupExeBuilder:
    """真正的 setup.exe 安装包构建器"""
    ...
    def create_setup_python_script(self, temp_dir):
        """创建 Python 安装脚本"""
...
def main():
    ...
    # 获取安装目录
    install_dir = input("请输入安装目录 (默认: C:\\Program Files\\{self.app_name}): ").strip()
    if not install_dir:
        install_dir = r"C:\Program Files\{self.app_name}"
    ...
    try:
        ...
        for sample_file in self.sample_data_files:
            src_file = Path(__file__).parent / "示例数据" / sample_file.name
            dst_file = os.path.join(sample_dir, sample_file.name)
            if src_file.exists():
                shutil.copy2(src_file, dst_file)
...
'''
        ...
        # 创建开始菜单快捷方式
        start_menu_dir = os.path.join(os.environ.get('ProgramData', ''), 'Microsoft', 'Windows', 'Start Menu', 'Programs', self.app_name)
...
' 创建开始菜单快捷方式
sLinkFile = "{start_menu_dir}\\\\{self.app_name}.lnk"
...
' 创建桌面快捷方式
sDesktopLink = "{desktop_dir}\\\\{self.app_name}.lnk"
...
'''
        ...
        print(f"✅ 创建 Python 安装脚本: {script_file}")
        return script_file
...
"""
    ...
    def build_setup_exe(self, temp_dir):
        ...
        # 创建 Python 安装脚本
        setup_script = self.create_setup_python_script(temp_dir)
...
def main():
    """主函数"""
    print("🔧 真正的 Windows setup.exe 安装包构建工具")
    print("创建真正的 Windows 可执行安装程序")
    print("-" * 60)
