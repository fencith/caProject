#!/usr/bin/env python3
"""
创建 Windows setup.exe 安装包（使用 pynsist）
Create Windows setup.exe Installer (using pynsist)
使用 pynsist 创建专业的 Windows 安装程序
"""
...
class SetupExePynsistBuilder:
    """setup.exe 安装包构建器（pynsist 版本）"""
...
'''
    ...
    def build_setup_exe(self, temp_dir):
        ...
        try:
            ...
            if build_dir.exists():
                ...
                if installer_files:
                    # 复制安装程序到项目目录
                    shutil.copy2(installer_files[0], setup_exe_file)
                    print(f"  ✅ setup.exe 生成成功: {setup_exe_file.name}")
                    return setup_exe_file
            
            print("  ❌ 未找到生成的 setup.exe 文件")
            return False
    ...
    def build_installer(self):
        ...
        # 检查 pynsist
        if not self.pynsist_available:
            print("❌ 构建失败：未找到 pynsist")
            print("请先安装 pynsist: pip install pynsist")
            return False
    ...
    def show_build_summary(self, setup_exe_file):
        ...
        print("\n💡 pynsist setup.exe 安装程序特点:")
        print("- 自动包含 Python 运行时")
        print("- 自动安装所有依赖")
        print("- 创建专业的 Windows 安装体验")
        print("- 无需用户预先安装 Python")


def main():
    """主函数"""
    print("🔧 Windows setup.exe 安装包构建工具（pynsist 版本）")
    print("使用 pynsist 创建专业的 Windows 安装程序")
    print("-" * 60)
    ...
    # 创建构建器并构建安装程序
    builder = SetupExePynsistBuilder()
