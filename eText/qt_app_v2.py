...
LOGO_PATH = r"D:\001\eText\spic_logo.png"
...
class EFileViewer(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowIcon(QIcon('spic_icon.ico'))
        ...
        self.export_pdf_button.setMinimumWidth(100)
        self.export_pdf_button.setEnabled(False)
        self.export_pdf_button.clicked.connect(self.exportPDF)
        ...
        self.statusBar.showMessage("国家电投云南国际  昆明生产运营中心 版权所有 | Ver: 2.0 | 联系电话: 0871-65666603")
    ...
    def exportPDF(self):
        ...
        filepath, _ = QFileDialog.getSaveFileName(self, "保存PDF报告", "", "PDF文件 (*.pdf);;所有文件 (*.*)")

        if filepath:
            try:
                self.createPDFReport(filepath)
                QMessageBox.information(self, "成功", "PDF报告已成功导出！")
            except Exception as e:
                QMessageBox.critical(self, "错误", "PDF导出失败:\n" + str(e))

    def createPDFReport(self, filepath):
        """创建PDF报告"""
        ...
        # 标题
        story.append(Paragraph("南网102-e文件解析工具", title_style))
        story.append(Paragraph("国家电投云南国际  昆明生产运营中心", subtitle_style))
        story.append(Paragraph("根据《南方电网新能源数据上送规范V3.3修订版》解析e文件", subtitle_style))
        story.append(Spacer(1, 20))

        # 文件信息
        story.append(Paragraph("文件信息", styles['Heading2']))
        file_data = [
            ["参数", "值"],
            ["文件名", os.path.basename(self.current_result["filename"])],
            ["解析时间", self.current_result["parse_time"]],
            ["System", self.current_result["header"].get("System", "N/A")],
            ["Version", self.current_result["header"].get("Version", "N/A")],
            ["Code", self.current_result["header"].get("Code", "N/A")],
            ["Data", self.current_result["header"].get("Data", "N/A")]
        ]
        file_table = Table(file_data, colWidths=[150, 400])
        ...
        stats_data = [
            ["统计项", "值"],
            ["数据段数量", str(total_sections)],
            ["数据表数量", str(total_tables)],
        ...
        # 数据段信息
        story.append(Paragraph("数据段详细信息", styles['Heading2']))
        
        for idx, section in enumerate(self.current_result["sections"], 1):
            # 数据段标题
            story.append(Paragraph(f"数据段 {idx}: {section['type']}", styles['Heading3']))
            ...
            section_data = [
                ...
                ["日期", section["date"]],
                ["时间", section["time"]],
                ["数据表数量", str(len(section["tables"]))],
                ["总行数", str(sum(t["row_count"] for t in section["tables"]))]
            ]
            section_table = Table(section_data, colWidths=[150, 400])
            ...
            # 数据表信息
            for table_idx, table in enumerate(section["tables"], 1):
                story.append(Paragraph(f"数据表 {table_idx} ({table['row_count']} 行)", styles['Heading4']))
                
                # 表头
                if table["header"]:
                    header_data = [["表头:"] + table["header"]]
                    header_table = Table(header_data, colWidths=[80] + [450/len(table["header"])] * len(table["header"]))
                ...
                if table["row_count"] > 0:
                    # 显示前10行或所有行（如果少于10行）
                    display_rows = min(table["row_count"], 10)
                    
                    # 创建数据行
                    table_data = []
                    for row_idx in range(display_rows):
                        row_data = table["rows"][row_idx]
                        row_values = [str(row_idx + 1)] + [str(row_data.get(header, "")) for header in table["header"]]
                        table_data.append(row_values)
                    
                    # 添加表头
                    table_data.insert(0, ["行号"] + table["header"])
                    
                    # 创建表格
                    data_table = Table(table_data, colWidths=[50] + [400/len(table["header"])] * len(table["header"]))
                    ...
                    if table["row_count"] > 10:
                        story.append(Paragraph(f"... 还有 {table['row_count'] - 10} 行数据", styles['Normal']))
                
                story.append(Spacer(1, 10))

            if idx < len(self.current_result["sections"]):
                story.append(Spacer(1, 20))

        # 页脚
        story.append(Spacer(1, 30))
        story.append(Paragraph("国家电投云南国际  昆明生产运营中心 版权所有", styles['Normal']))
        story.append(Paragraph("版本: 2.1.2 | 作者：陈丰 电话：0871-65666603", styles['Normal']))
        story.append(Paragraph("报告生成时间: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"), styles['Normal']))
